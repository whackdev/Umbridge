	...
	async execute(interaction) {
		try {
          ...
          JSON.stringify(standardArray.sort((a, b) => a - b));
				...
				const fieldsArray = rollMessage.embeds[0].fields;
				const rollReaction = await rollMessage.reactions.cache.map(r => r.emoji.toString())
          ...
          submittedTotal === messageRollTotal && received == expected;
			...
			return interaction.editReply(`There was a problem executing this command: ${error.message}\n${error.stack}`);
		}
	},
};
